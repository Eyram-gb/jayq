...
const Others = () => {
  ...
  return (
    <Layout>
          ...
          {imageNames.map((item) => {
            return (
              <div className='w-full group relative cursor-pointer' key={item} onClick={() => openModal(item)}>
                <img src={`/Others/${item}.jpg`} alt="artwork" className=' w-full transition-all duration-150 group-hover:blur-[4px]' />
      ...
      {selectedImage && (
        <ImageModal imageUrl={`/Others/${selectedImage}.jpg`} onClose={closeModal} />
      )}
    </Layout>
  )
}
